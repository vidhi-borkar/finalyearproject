...
def move_all_legs_forward2():
    for leg in ['L2', 'L4','L6']:
        pca, coxa_ch, femur_ch, tibia_ch = legs[leg]
        set_servo_pulse_us(pca, femur_ch, FWD_FEM)  # Femur (lift)
        set_servo_pulse_us(pca, tibia_ch, REV_TIB)  # Tibia (extend)
    time.sleep(0.5)
    stop_all_legs()
...
def move_all_legs_backward2():
    for leg in ['L2', 'L4', 'L6']:
        pca, coxa_ch, femur_ch, tibia_ch = legs[leg]
        set_servo_pulse_us(pca, femur_ch, REV_FEM)  # Femur (lower)
        set_servo_pulse_us(pca, tibia_ch, FWD_TIB)  # Tibia (retract)
    time.sleep(0.5)
    stop_all_legs()
...
# Turn left by moving coxa joints asymmetrically
def turn_left1():
    for leg in ['L1', 'L3', 'L5']:  # Left legs
...
# Turn right by reversing left/right coxa movement
def turn_right1():
    for leg in ['L1', 'L3', 'L5']:
        pca, coxa_ch, _, _ = legs[leg]
        set_servo_pulse_us(pca, coxa_ch, REV_COXA)
...
try:
    ...
    #for moving forward
        ...
        move_all_legs_forward1()
        time.sleep(1)
#       print("Coxa Forward")
        move_all_coxa_forward1()
        time.sleep(1)
   #    print("Tripod Step Backward")
        move_all_legs_backward1()
        time.sleep(1)
    #   print("Coxa Backward")
        move_all_coxa_backward1()    
        time.sleep(1)           
     #  print("Tripod Step Forward")
        move_all_legs_forward2()
        time.sleep(1)
      # print("Coxa Forward")
        move_all_coxa_forward2()
        time.sleep(1)
      # print("Tripod Step Backward")
        move_all_legs_backward2()
        time.sleep(1)
       #print("Coxa Backward")
        move_all_coxa_backward2()
        time.sleep(1)
